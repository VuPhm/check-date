...
const storeCode = ref(''); const password = ref(''); const joinCode = ref('');
...
async function signIn() {
  try { busy.value = true; saveHost(); const session = mode.value === 'manager'
    ...
    appStore.setSession(session); password.value = ''; joinCode.value = ''; message.value = mode.value === 'manager' ? 'Đã đăng nhập CHT.' : 'Đã tham gia cửa hàng.'; await refreshAdmin(); await appStore.syncNow();
...
<template>
  <section class="sidebar-section sync-config-section"><h4 class="sidebar-section-title">Cửa hàng & đồng bộ</h4>
    ...
    <template v-if="!appStore.session">
      ...
      <label class="sync-config-field"><span>Mã cửa hàng</span><input v-model="storeCode" class="form-input" inputmode="numeric" maxlength="4" @input="storeCode = cleanCode(storeCode)"></label>
      <template v-if="mode === 'manager'"><label class="sync-config-field"><span>Mật khẩu CHT</span><input v-model="password" type="password" class="form-input"></label></template>
